...
namespace Messer.TaxiBooker.Api.Domain.IdentityManager
{
        ...
        where TUser : IdentityUser, new()
    {
        ...
        public virtual async Task<IdentityManagerResult<CreateResult>> CreateUserAsync([NotNull] IEnumerable<PropertyValue> properties)
        {
            ...
            var emailConfirmedClaim = properties.Single(x => x.Type == Constants.ClaimTypes.EmailConfirmed);

            var name = nameClaim.Value;
            var email = emailClaim.Value;
            var password = passwordClaim.Value;

            string[] exclude = { Constants.ClaimTypes.Email, Constants.ClaimTypes.Password, Constants.ClaimTypes.Name,Constants.ClaimTypes.EmailConfirmed };
            ...
            var user = new TUser { UserName = email };
            user.Email = email;
            user.EmailConfirmed=Convert.ToBoolean(emailConfirmedClaim.Value);
            ...
            //Add claim Name  used for display name
            result = await userManager.AddClaimAsync(user, new Claim(Constants.ClaimTypes.Name, name));
            if (!result.Succeeded)
            {
                return new IdentityManagerResult<CreateResult>(result.Errors.Select(x => x.Description).ToArray());
            }
        ...
        public virtual IdentityManagerMetadata GetStandardMetadata(bool includeAccountProperties = true)
        {
            ...
            {
                new PropertyMetadata(){Name="Name",Type=Constants.ClaimTypes.Name,DataType=PropertyDataType.String,Required=true },
        ...
        public virtual bool GetIsEmailConfirmed(TUser user)
        {
            return this.userManager.IsEmailConfirmedAsync(user).Result;
        }
        public virtual IdentityManagerResult SetEmailConfirmed(TUser user, bool confirmed)
        {
            if (confirmed)
            {
                var token = this.userManager.GenerateEmailConfirmationTokenAsync(user).Result;
                var result = this.userManager.ConfirmEmailAsync(user, token).Result;
                if (!result.Succeeded)
                {
                    return new IdentityManagerResult(result.Errors.Select(x => x.Description).First());
                }
        ...
        public virtual async Task<IdentityManagerResult> AddUserRoleAsync(string subject, string value)
        {
            ...
            if (!existingRoles.Any(x => (x == value)))
            {
                var result = await this.userManager.AddToRoleAsync(user, value);
                if (!result.Succeeded)
                {
                    return new IdentityManagerResult<CreateResult>(result.Errors.Select(x => x.Description).ToArray());
                }
        ...
        public virtual async Task<IdentityManagerResult> RemoveUserRoleAsync(string subject, string value)
        {
            var user = await userManager.FindByIdAsync(subject);
            if (user == null)
            {
            ...
            if (existingRoles.Any(x => (x == value)))
            {
                var result = await this.userManager.RemoveFromRoleAsync(user, value);
                if (!result.Succeeded)
                {
                    return new IdentityManagerResult<CreateResult>(result.Errors.Select(x => x.Description).ToArray());
                }
